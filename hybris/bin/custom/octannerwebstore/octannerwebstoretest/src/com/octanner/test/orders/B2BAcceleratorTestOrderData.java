 ...
 *  
 */
package com.octanner.test.orders;
...
import com.octanner.test.constants.OctannerwebstoreTestConstants;
import com.octanner.test.services.AccountManagerApproveScheduleService;
...
public class B2BAcceleratorTestOrderData
{
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(OctannerwebstoreTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(OctannerwebstoreTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(OctannerwebstoreTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(OctannerwebstoreTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(OctannerwebstoreTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(OctannerwebstoreTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(OctannerwebstoreTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(OctannerwebstoreTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(OctannerwebstoreTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(OctannerwebstoreTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(OctannerwebstoreTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(OctannerwebstoreTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(OctannerwebstoreTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(OctannerwebstoreTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
	...
	protected String placeReplenishmentOrderInCancelledState(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			code = scheduleOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerwebstoreTestConstants.STANDARD_COSTCENTER, null, triggerData).getJobCode();
			if (code != null)
			{
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? OctannerwebstoreTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderRequiringApproval(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerwebstoreTestConstants.STANDARD_COSTCENTER, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerwebstoreTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerwebstoreTestConstants.LOWLIMIT_COSTCENTER, null);
	...
	{
		// Lookup the site
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(OctannerwebstoreTestConstants.POWERTOOLS_SITE);
		// Lookup the customer
			...
			final String purchaseOrderNumber, final String costCenterCode, final String quotePetition)
	{
		...
		final ImpersonationContext ctx = new ImpersonationContext();
		ctx.setSite(getCmsAdminSiteService().getSiteForId(OctannerwebstoreTestConstants.POWERTOOLS_SITE));
		ctx.setUser(customer);
			...
			final B2BCustomerModel customer)
	{
		LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(),
				OctannerwebstoreTestConstants.POWERTOOLS_SITE));
		...
		if (containsOrder(orderList, purchaseOrderNumber))
		{
			LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ",
					purchaseOrderNumber, customer.getUid(), OctannerwebstoreTestConstants.POWERTOOLS_SITE));
	...
	{
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(OctannerwebstoreTestConstants.POWERTOOLS_SITE);
		final B2BCustomerModel customer = getUserService().getUserForUID(OctannerwebstoreTestConstants.APPROVER_UID.toLowerCase(),
